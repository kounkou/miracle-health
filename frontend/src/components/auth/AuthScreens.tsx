...
export function LoginForm({ onSwitch, onLogin }: { onSwitch: () => void; onLogin: (token: string, email: string, expiresAt?: string) => void }) {
    const [email, setEmail] = useState("");
    const [password, setPassword] = useState("");
    ...
    async function submit(e: React.FormEvent) {
        ...
        try {
            ...
            onLogin(data.token, data.email, data.expiresAt);
        } catch (err) {
            setError(err instanceof Error ? err.message : String(err));
        } finally {
            setLoading(false);
