...
export const calculateReadinessScore = (baselineMetrics: ReadinessInputs, schedule: TimelineScheduleStep): ScoreResult => {
    ...
    const baseRaw = 100.0 / (1.0 + Math.exp(-kFactor * (tsb + tsbOffset)));
    const sched = schedule;
    let hiitPenalty = 0.0, zone2Penalty = 0.0;
    if (sched.daysHiit !== undefined && sched.daysHiit <= hiitWindow) {
      hiitPenalty = baseHiit * Math.max(0.0, 1.0 - (sched.daysHiit/hiitWindow));
    }
    if (sched.daysZone2 !== undefined && sched.daysZone2 <= zone2Window) {
      zone2Penalty = baseZone2 * Math.max(0.0, 1.0 - (sched.daysZone2/zone2Window));
    }
